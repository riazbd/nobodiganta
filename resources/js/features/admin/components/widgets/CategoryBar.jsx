export function CategoryBar({ items = [], lang = 'bn' }) {
  const colorMap = {
    red: '#e8001e', blue: '#3b82f6', green: '#10b981', orange: '#f59e0b',
      ...
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-2.5">
          ...
          <div className="flex-1 h-1.75 bg-[#f3f4f6] rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${item.pct}%`, backgroundColor: colorMap[item.color] || '#e8001e' }} />
          </div>
