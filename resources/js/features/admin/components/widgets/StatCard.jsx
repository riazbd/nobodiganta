export function StatCard({ icon: Icon, label, value, change, changeUp, linkText, onLinkClick, color = 'red' }) {
  const colorMap = {
    red: { icon: 'bg-[#eceff1]', text: 'text-[#263238]' },
  ...
  return (
    <div className={`bg-[var(--card-bg,#ffffff)] border border-[var(--card-border,#e8ebf4)] rounded-xl p-5 shadow-sm relative overflow-hidden cursor-default transition-all hover:-translate-y-0.5 hover:shadow-md`}>
      <div className={`absolute top-0 right-0 w-20 h-20 rounded-tr-xl rounded-bl-[80px] opacity-[0.07] bg-[var(--${color},#263238)]`} />
      ...
      {change && (
        <div className={`text-[11.5px] font-semibold flex items-center gap-1 ${changeUp ? 'text-[#10b981]' : 'text-[#263238]'}`}>
          {changeUp ? '▲' : '▼'} {change}
        </div>
      )}
      {linkText && onLinkClick && (
        <button onClick={onLinkClick} className="text-[11.5px] text-[#263238] font-semibold inline-flex items-center gap-1 mt-1.5 hover:underline">
          {linkText} →
        </button>
      )}
    </div>
  ...
  return (
    <div className="bg-[var(--card-bg,#ffffff)] border border-[var(--card-border,#e8ebf4)] rounded-xl px-4.5 py-3.75 flex items-center gap-3.5 shadow-sm transition-all hover:shadow-md">
      ...
      <div>
        ...
        {change && <div className={`text-[11px] font-semibold mt-0.5 ${changeColor === 'green' ? 'text-[#10b981]' : changeColor === 'red' ? 'text-[#263238]' : 'text-[#3b82f6]'}`}>{change}</div>}
      </div>
    </div>
  );
}
