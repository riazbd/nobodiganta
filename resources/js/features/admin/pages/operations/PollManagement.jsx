               ...
               {poll.options?.map(opt => (
                 <div key={opt.id} className="bg-gray-50 rounded-2xl p-4">
                    <div className="flex justify-between text-sm mb-2">
                       ...
                       <span className="font-black text-[#263238]">{opt.votes}</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                       <div 
                         className="bg-[#263238] h-full transition-all duration-500" 
            ...
            <div className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
               <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={form.question_bn} onChange={e => setForm({...form, question_bn: e.target.value})} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#263238]" />
                  </div>
                  <div>
               ...
               <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="date" value={form.start_date} onChange={e => setForm({...form, start_date: e.target.value})} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#263238]" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5 uppercase">End Date (Optional)</label>
                    <input type="date" value={form.end_date} onChange={e => setForm({...form, end_date: e.target.value})} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#263238]" />
                  </div>
               </div>
               ...
               <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-gray-50 pb-2">
                    ...
                    <button onClick={addOption} className="text-xs font-bold text-[#263238] hover:underline">+ Add Option</button>
                  </div>
                  {form.options.map((opt, idx) => (
                    <div key={idx} className="grid grid-cols-2 gap-4">
                       <input type="text" value={opt.option_bn} onChange={e => updateOption(idx, 'option_bn', e.target.value)} className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#263238]" placeholder={`Option ${idx+1} (BN)`} />
                       <input type="text" value={opt.option_en} onChange={e => updateOption(idx, 'option_en', e.target.value)} className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#263238]" placeholder={`Option ${idx+1} (EN)`} />
                    </div>
                  ))}
               </div>
