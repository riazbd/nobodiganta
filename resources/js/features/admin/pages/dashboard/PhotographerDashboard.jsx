      ...
      <div className="flex items-start justify-between mb-5.5">
        <div>
          <h1 className="text-xl font-bold text-[var(--text-primary,#1a1d2e)] flex items-center gap-2 font-['Noto_Sans_Bengali']">
            <Camera className="w-5 h-5 text-[#263238]" />
        ...
        <div className="flex items-center gap-2.5">
          <button onClick={() => showToast(lang === 'bn' ? 'মিডিয়া আপলোড খুলছে...' : 'Opening media upload...')} className="bg-[#263238] text-white rounded-lg px-4 py-2 text-[12.5px] font-semibold flex items-center gap-1.5 hover:bg-[#1a2428] transition-colors">
      ...
      <div className="bg-[var(--card-bg,#ffffff)] border border-[var(--card-border,#e8ebf4)] rounded-xl shadow-sm overflow-hidden mb-4.5">
        <div className="px-5 py-4 border-b border-[var(--card-border,#e8ebf4)] flex items-center justify-between">
          <h3 className="text-sm font-bold flex items-center gap-2">
            <Image className="w-4 h-4 text-[#263238]" />
            {lang === 'bn' ? 'সাম্প্রতিক মিডিয়া' : 'Recent Media'}
          </h3>
          <button onClick={() => onNavigate?.('media')} className="bg-[#263238] text-white text-[11px] font-semibold px-3 py-1.25 rounded-md hover:bg-[#1a2428] transition-colors">
